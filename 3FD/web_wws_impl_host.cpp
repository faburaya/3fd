...
namespace wws
{
    ...
    /// <summary>
    /// Gets a copy of the policy description implemented by wsutil.
    /// </summary>
    /// <param name="heap">The heap to allocate memory from.</param>
    /// <returns>The copy of the policy description implemented by wsutil.</returns>
    void * SvcEndptBindHttpUnsecImpls::GetPolicyDescription(WSHeap &heap)
    {
        /* Initialization of this data member is delayed. The policy description
        has to copied, because the original one cannot be modified : */
        if (m_copyOfPolicyDescription == nullptr)
        {
            m_copyOfPolicyDescription = heap.Alloc<WS_HTTP_POLICY_DESCRIPTION>();
            *m_copyOfPolicyDescription = *m_policyDescription;
        }

        return m_copyOfPolicyDescription;
    }
    ...
    /// <param name="heap">The heap to allocate memory from.</param>
    /// <returns>The copy of the policy description implemented by wsutil.</returns>
    void * SvcEndptBindHttpSslImpls::GetPolicyDescription(WSHeap &heap)
    {
        /* Initialization of this data member is delayed. The policy description
        has to copied, because the original one cannot be modified : */
        if (m_copyOfPolicyDescription == nullptr)
        {
            m_copyOfPolicyDescription = heap.Alloc<WS_HTTP_SSL_POLICY_DESCRIPTION>();
            *m_copyOfPolicyDescription = *m_policyDescription;
        }
    ...
    /// <param name="heap">The heap to allocate memory from.</param>
    /// <returns>The copy of the policy description implemented by wsutil.</returns>
    void * SvcEndptBindHttpHeaderAuthSslImpls::GetPolicyDescription(WSHeap &heap)
    {
        /* Initialization of this data member is delayed. The policy description
        has to copied, because the original one cannot be modified : */
        if (m_copyOfPolicyDescription == nullptr)
        {
            m_copyOfPolicyDescription = heap.Alloc<WS_HTTP_SSL_HEADER_AUTH_POLICY_DESCRIPTION>();
            *m_copyOfPolicyDescription = *m_policyDescription;
        }
                                            ...
                                            WSError &err)
    {
        ...
        /* The original array of channel properties resides in wsutil generated
        implementation, hence cannot be altered. This will make a copy of the
        properties and extend it with the provided additional channel properties: */

        auto &wsutilChannelProps = GetChannelProperties(heap);
        auto &extendableChannelProps = wsutilChannelProps;
        auto nBytesWsutilPropsArray = wsutilChannelProps.propertyCount * sizeof(WS_CHANNEL_PROPERTY);
        auto extendedPropCount = wsutilChannelProps.propertyCount + moreChannelProps.propertyCount;
        auto nBytesExtPropsArray = extendedPropCount * sizeof(WS_CHANNEL_PROPERTY);

        // first copy the channel properties generated by wsutil:
        extendableChannelProps.properties =
            (WS_CHANNEL_PROPERTY *)memcpy(
                heap.Alloc(nBytesExtPropsArray),
                wsutilChannelProps.properties,
                nBytesWsutilPropsArray
            );

        // then add the ones coming from the parameter:
        memcpy(
            extendableChannelProps.properties + wsutilChannelProps.propertyCount,
            moreChannelProps.properties,
            nBytesExtPropsArray - nBytesWsutilPropsArray
        );
    ...
    /// </remarks>
    std::shared_ptr<BaseSvcEndptBindImpls> ServiceBindings::GetImplementations(const string &bindName) const
    {
    ...
    {
        m_bindNameToImpls[bindName].reset(
            new SvcEndptBindHttpHeaderAuthSslImpls(
                functionTable,
                contractDescription,
                policyDescription
            )
...
#			endif
    ...
    void WebServiceHost::Setup(
        ...
        bool enableMEX)
    {
        m_pimpl->Setup(wsdFilePath, config, authzCallback, enableMEX);
    }
