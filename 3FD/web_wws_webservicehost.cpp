...
namespace _3fd
{
	namespace web
	{
		namespace wws
		{
			...
			/// </remarks>
			const ServiceBindings::Implementation *
			ServiceBindings::GetImplementation(const string &bindName) const
			{
				auto iter = m_bindToImpl.find(bindName);

				if (m_bindToImpl.end() != iter)
					return &iter->second;
				else
					return nullptr;
			}
				...
				std::vector<SvcEndpointInfo> &endpointsInfo)
			{
				...
				try
				{
					...
					for (unsigned long idx = 0; idx < portNodes->length(); ++idx)
					{
						...
						auto svcImplForBind = bindings.GetImplementation(endpoint.bindingName);

						if (svcImplForBind != nullptr)
							endpoint.implementation = *svcImplForBind;
						else
                ...
                WSHeap &heap)
            {
                ...
                try
                {
                    ...
                    if (enableMEX)
                    {
						endptPropCount = 5; // count of properties is later needed
						...
						// URL suffix to get metadata through MEX:
						endpointProps[idxProp++] = WS_SERVICE_ENDPOINT_PROPERTY {
					...
					for (auto &epnfo : endptsInfo)
					{
						...
						auto hr = epnfo.implementation.createEndpoint(
							epnfo.implementation.wsCallbackCreateEndpoint, // generated by wsutil.exe
                            epnfo.address,
							epnfo.implementation.functionTable, // implemented by client code
							copyOfEndptProps,
							endptPropCount,
                            heap,
                            &endpoint,
                        ...
                        {
                            std::ostringstream oss;
							oss << "Failed to create web service endpoint at " << epnfo.address;
