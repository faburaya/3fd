	...
	namespace web
	{
		namespace wws
		{
            ...
            void * SvcEndptBindHttpUnsecImpls::GetPolicyDescription(WSHeap &heap)
            {
                ...
                if (m_copyOfPolicyDescription == nullptr)
                {
                    ...
                    *m_copyOfPolicyDescription = *m_policyDescription;
                    ...
                    auto numBytesPropsArray =
                        m_policyDescription->channelProperties.propertyCount * sizeof(WS_CHANNEL_PROPERTY);

                    m_copyOfPolicyDescription->channelProperties.properties =
                        (WS_CHANNEL_PROPERTY *) memcpy(
                            heap.Alloc(numBytesPropsArray),
                            m_policyDescription->channelProperties.properties,
                            numBytesPropsArray
                        );
                }
                                                    ...
                                                    WSError &err)
            {
                ...
                encoding, which is usually the default implementation generated
                by wsutil. */
                
                auto channelPropertiesBegin = GetChannelProperties(heap).properties;
                auto channelPropertiesEnd = channelPropertiesBegin + GetChannelProperties(heap).propertyCount;
                ...
                iter->value = WS_HEAP_NEW(heap, WS_ENCODING, (WS_ENCODING_XML_MTOM_UTF16LE));
                iter->valueSize = sizeof WS_ENCODING;
