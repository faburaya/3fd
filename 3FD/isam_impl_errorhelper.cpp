	...
	namespace isam
	{
			...
			const char *what)
		{
			...
			else
			{
				...
				else // But a warning will add an entry to the log output:
					core::Logger::Write(
			...
			const std::function<string(void)> &what)
		{
			...
			try
			{
				std::array<wchar_t, 128> ucs2text;
			...
			catch (std::exception &ex)
			{
				...
				oss << "Microsoft ESE API returned error code " << errorCode
					<< ". Another failure prevented proper error details to be loaded: " << ex.what();
				return core::AppException<std::exception>(what().c_str(), oss.str());
			}
		}
