...
namespace wws
{
    ...
    /// <summary>
    /// Creates from a string view a heap allocated WS_STRING structure.
    /// </summary>
    /// <param name="str">The original string view.</param>
    /// <param name="heap">The heap to allocate memory from.</param>
    /// <returns>
    /// A pointer to a heap allocated WS_STRING structure
    /// referencing the data in the original string view.
    /// </returns>
    ...
    // Analogous to implementation for UTF-8, but without conversion
    WS_STRING ToWsString(const std::wstring_view &str, WSHeap &heap)
    {
        WS_STRING obj;
        obj.length = str.length();
        obj.chars = heap.Alloc<wchar_t>(obj.length);
        wmemcpy(obj.chars, str.data(), str.length());
        return obj;
    }
        ...
        WS_ERROR *wsErrorHandle)
    {
        ...
        try
        {
            ...
            static const WS_STRING faultReasonLanguage = ToWsString("en", heap);
